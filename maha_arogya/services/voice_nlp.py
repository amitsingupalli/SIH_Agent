...
Integrates with Bhashini API / Whisper for Marathi and Hindi speech recognition,
...
# Clinical Symptom Mapping: Marathi & Hindi to Standard Clinical Terminology
...
class VernacularVoiceNLPService:
    """Service to process speech/text in Marathi & Hindi, normalize digits, and extract clinical entities."""
    ...
    def extract_clinical_entities(self, transcript: str) -> ExtractedClinicalEntities:
        ...
        and clinical symptoms with SNOMED codings from Marathi or Hindi transcripts.
        """
        normalized = self.normalize_devanagari_numbers(transcript)

        # 1. Extract Patient ID
        patient_id = "PAT-UNKNOWN"
        pid_match = re.search(r'(?:PAT|PATIENT|ID|\u0930\u0941\u0917\u094d\u0923|\u092a\u0947\u0936\u0902\u091f)[\s:-]*([A-Za-z0-9-]+)', normalized, re.IGNORECASE)
        if pid_match:
        ...
        bp_patterns = [
            r'(\d{2,3})\s*(?:/|\u0935\u0930|\u092c\u093e\u092f|by|\s)\s*(\d{2,3})',
            r'(?:\u092c\u0940\u092a\u0940|\u0930\u0915\u094d\u0924\u0926\u093e\u092c)[\s:-]*(\d{2,3})[\s/]+(\d{2,3})'
        ]
        ...
        gestation = 0
        w_match = re.search(r'(\d{1,2})\s*(?:\u0906\u0920\u0935\u0921\u0947|\u0939\u092b\u094d\u0924\u0947|weeks)', normalized, re.IGNORECASE)
        if w_match:
            gestation = int(w_match.group(1))
        else:
            m_match = re.search(r'(\d{1})\s*(?:\u092e\u0939\u093f\u0928\u0947|\u092e\u0939\u0940\u0928\u0947|months)', normalized, re.IGNORECASE)
            if m_match:
        ...
        for vernacular_kw, data in CLINICAL_SYMPTOMS_MAP.items():
            if vernacular_kw in normalized:
    ...
    def generate_vernacular_referral_sms(
        ...
        """Generates an official Marathi SMS / WhatsApp alert for rural patients and ASHA workers."""
        return (
    ...
    def generate_vernacular_escalation_sms(
        ...
        """Generates a 48-Hour SLA Breach Marathi Escalation Notice."""
        return (
